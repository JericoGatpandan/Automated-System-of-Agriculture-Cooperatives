...
import { JWT_EXPIRES_IN, JWT_SECRET, PORT } from "./config/env.config.js";
...
app.post("/api/auth/login", (req, res) => {
  ...
  if (password !== "password") {
    return res
      .status(401)
      .json({ message: "Invalid credentials. Use 'password' for testing." });
  }
...
app.get("/api/auth/me", (req, res) => {
  ...
  try {
    ...
    // Mock user response
    res.json({
      user: { id: decoded.id, email: "mock@user.com", role: decoded.role },
    });
  } catch (err) {
