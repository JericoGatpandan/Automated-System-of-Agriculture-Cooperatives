    ...
    include: [
      {
        ...
        attributes: ["coopName", "primaryCoopID"],
      },
    ],
  },
  {
...
router.get("/my-coop", authenticate, authorize("Officer"), async (req, res) => {
  try {
    ...
    const farmers = await db.Farmer.findAll({
      where: { farmerID: { [Op.in]: farmerIDs }, isDeleted: false },
      include: FARMER_INCLUDES,
      order: [
        ["lastName", "ASC"],
        ["firstName", "ASC"],
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role === "Officer") {
        const coop = await getOfficerCoop(req.user.userID);
        if (!coop)
          return res.status(403).json({ message: "Cooperative not found" });
...
// ─────────────────────────────────────────────────────────
router.put("/:id", authenticate, authorize("Officer"), async (req, res) => {
  try {
    const coop = await getOfficerCoop(req.user.userID);
    if (!coop)
      return res.status(404).json({ message: "Cooperative not found" });
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (validIDs.length === 0) {
        ...
        return res
          .status(403)
          .json({ message: "None of the farmers belong to your cooperative" });
      }
      ...
      await db.Farmer.update(
        { isDeleted: true },
        { where: { farmerID: validIDs }, transaction: t },
      );
      await db.User.update(
        { isDeleted: true },
        { where: { userID: userIDs }, transaction: t },
      );
...
// ─────────────────────────────────────────────────────────
router.get("/crop-types", authenticate, async (_req, res) => {
  try {
