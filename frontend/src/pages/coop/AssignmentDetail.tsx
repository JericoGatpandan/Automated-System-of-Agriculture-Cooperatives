...
export function AssignmentDetail() {
  ...
  const pct = assignment?.quantityRequired
    ? Math.min(
        100,
        Math.round((totalCommitted / assignment.quantityRequired) * 100),
      )
    : 0;
  const fulfillments = assignment?.FarmerFulfillments || [];
  const totalPages = Math.max(1, Math.ceil(fulfillments.length / pageSize));
  const paginatedFulfillments = fulfillments.slice(
  ...
  const updateFulfillmentStatus = async (
    ...
    try {
      await axios.put(`${API}/${id}/fulfillments/${fulfillmentID}`, {
        status: newStatus,
      });
      await fetchAssignment();
    } catch {
  ...
  const handleRemove = async () => {
    ...
    try {
      await axios.delete(
        `${API}/${id}/fulfillments/${removeTarget.fulfillmentID}`,
      );
      setRemoveOpen(false);
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold">
              ORD-{order?.orderID || assignment.orderID}
            </h1>
            <p className="text-sm text-muted-foreground">
              {order?.buyerName || "—"} — {order?.CropType?.cropName || "—"}
            </p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-4 gap-x-6 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">
                  {order?.CropType?.cropName || "—"}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Qty Required</p>
                <p className="font-medium font-mono">
                  {assignment.quantityRequired?.toLocaleString()} kg
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Cooperative</p>
                <p className="font-medium">
                  {assignment.PrimaryCooperative?.coopName || "—"}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Assigned</p>
                <p className="font-medium">
                  {fmtDate(assignment.assignedDate)}
                </p>
              </div>
              <div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription className="mt-1">
                  {assignment.FarmerFulfillments?.length || 0} farmer
                  {(assignment.FarmerFulfillments?.length || 0) !== 1
                    ? "s"
                    : ""}{" "}
                  assigned
                </CardDescription>
              ...
              <Button
                size="sm"
                onClick={openAssignFarmer}
                id="assign-farmer-btn"
              >
            ...
            <div className="flex items-center gap-4 mb-4 text-sm">
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-mono font-medium">
                  {assignment.quantityRequired?.toLocaleString()}
                </span>
              </div>
              ...
              <span
                className={`font-mono font-medium ${remaining > 0 ? "text-yellow-700" : "text-green-700"}`}
              >
              ...
              {fulfillments.length === 0 ? (
                ...
                <ScrollArea className="min-h-0 flex-1">
                  <Table>
                    ...
                    <TableBody>
                      {paginatedFulfillments.map((f: any) => (
                        <TableRow key={f.fulfillmentID}>
                          ...
                          <TableCell className="py-4 text-right font-mono">
                            {f.quantityCommitted?.toLocaleString()}
                          </TableCell>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="pt-2">
              Remove{" "}
              <strong>
                {removeTarget?.Farmer?.lastName},{" "}
                {removeTarget?.Farmer?.firstName}
              </strong>{" "}
