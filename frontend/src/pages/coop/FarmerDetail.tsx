        ...
        <div className="max-w-3xl mx-auto px-6 py-8">
          <Card className="border-destructive/50">
            <CardContent className="pt-6">
              <p className="text-sm text-destructive">{error || "Farmer not found"}</p>
              <Button variant="outline" size="sm" className="mt-2" onClick={() => navigate(backRoute)}>Back</Button>
            </CardContent>
          </Card>
        </div>
      </div>
    ...
    <div className="ml-64 min-h-screen bg-gray-50/50">
      <div className="w-full mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-4 mb-6">
          <Button variant="ghost" size="icon" onClick={() => navigate(backRoute)} id="back-to-list">
        ...
        <Tabs defaultValue="personal">
          <TabsList className="grid w-full grid-cols-4 mb-6">
            ...
            <TabsTrigger value="farm" className="text-xs sm:text-sm">Farm</TabsTrigger>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{farmer.User?.email || "—"}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <Badge variant="secondary" className="text-xs mt-0.5">Farmer</Badge>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{farmer.farmName || "—"}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{farmer.farmLocation}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <TabsContent value="memberships">
            <Card>
              ...
              <CardContent className="px-0 pb-0">
                {farmer.FarmerCooperatives && farmer.FarmerCooperatives.length > 0 ? (
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {farmer.FarmerCooperatives.map((fc, i) => (
                        <TableRow key={i}>
                          ...
                          <TableCell className="py-4">{fmtDate(fc.joinedDate)}</TableCell>
                  ...
                  <div className="text-center py-8">
                    <p className="text-sm text-muted-foreground">No cooperative memberships</p>
                  </div>
                )}
              </CardContent>
            </Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium">{fmtDate(farmer.createdAt)}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <Shield className="h-4 w-4 text-muted-foreground shrink-0" />
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-muted hover:bg-muted">
                          ...
                          <TableHead className="font-semibold text-muted-foreground text-right">Qty Available</TableHead>
                          <TableHead className="font-semibold text-muted-foreground">Grade</TableHead>
                      ...
                      <TableBody>
                        {farmer.Products.map((p) => (
                          <TableRow key={p.productID}>
                            <TableCell className="py-4 font-medium">{p.CropType?.cropName || "—"}</TableCell>
                            <TableCell className="py-4 text-right font-mono">PHP {Number(p.unitPrice).toFixed(2)}</TableCell>
                            <TableCell className="py-4 text-right font-mono">{p.availableQuantity}</TableCell>
                            <TableCell className="py-4">{p.qualityGrade}</TableCell>
                            <TableCell className="py-4">{fmtDate(p.updatedAt)}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
