...
import {
  Users, Plus, Eye, Pencil, Trash2, Search, AlertTriangle, Loader2,
} from "lucide-react";
...
export function CoopFarmerRegistry() {
  ...
  // Selection helpers
  const allSelected = filtered.length > 0 && filtered.every((f) => selected.has(f.farmerID));
  const toggleAll = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSelected((s) => { const n = new Set(s); n.delete(deleteFarmer.farmerID); return n; });
      await fetchFarmers();
    } catch (err: any) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await axios.post(`${API}/bulk-delete`, { farmerIDs: Array.from(selected) });
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Manage farmer memberships in your cooperative</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <Button onClick={() => navigate("/coop/farmers/new")} id="register-farmer-btn">
            ...
            <CardContent className="px-0 pb-0">
              {filtered.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  {!search && (
                    <Button variant="outline" className="mt-4" onClick={() => navigate("/coop/farmers/new")}>
                      <Plus className="h-4 w-4 mr-2" />
                          ...
                          <TableCell className="py-4 text-right">
                            <div className="flex justify-end gap-1">
                              <Button
                                ...
                                onClick={() => navigate(`/coop/farmers/${f.farmerID}`)}
                              ...
                              <Button
                                variant="ghost" size="icon"
                                onClick={() => navigate(`/coop/farmers/${f.farmerID}/edit`)}
                                id={`edit-farmer-${f.farmerID}`}
                              >
                              ...
                              <Button
                                ...
                                onClick={() => { setDeleteFarmer(f); setDeleteOpen(true); }}
                                id={`delete-farmer-${f.farmerID}`}
                              >
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="pt-2">
              Are you sure you want to deactivate <strong>{deleteFarmer ? farmerDisplayName(deleteFarmer) : ""}</strong>? This will deactivate the farmer account and their login credentials.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteOpen(false)} disabled={deleteLoading}>Cancel</Button>
            <Button variant="destructive" onClick={handleDelete} disabled={deleteLoading} id="confirm-delete-farmer">
              {deleteLoading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Deactivating…</> : "Deactivate"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <div className="flex items-center gap-3 mb-1">
              ...
              <DialogTitle>Deactivate {selected.size} Farmer{selected.size !== 1 ? "s" : ""}</DialogTitle>
            </div>
            <DialogDescription className="pt-2">
              Are you sure you want to deactivate <strong>{selected.size} farmer{selected.size !== 1 ? "s" : ""}</strong>? This will deactivate their accounts and login credentials.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setBulkDeleteOpen(false)} disabled={bulkDeleteLoading}>Cancel</Button>
            <Button variant="destructive" onClick={handleBulkDelete} disabled={bulkDeleteLoading} id="confirm-bulk-delete">
              {bulkDeleteLoading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Deactivating…</> : "Deactivate All"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
