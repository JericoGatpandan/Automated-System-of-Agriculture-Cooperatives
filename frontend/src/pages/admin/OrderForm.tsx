...
export function OrderForm() {
  ...
  useEffect(() => {
    axios.get(CROP_API).then((r) => setCropTypes(r.data.cropTypes)).catch(() => {});
  }, []);
  ...
  const canSubmit = buyerName && buyerContact && cropTypeID && requestedQuantity && urgencyLevel && orderDate;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        buyerName, buyerCompany: buyerCompany || null, buyerContact,
        cropTypeID: parseInt(cropTypeID), requestedQuantity: parseInt(requestedQuantity),
        urgencyLevel, orderDate, notes: notes || null,
      };
      if (isEdit) {
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-2xl mx-auto px-6 py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-xl font-bold text-foreground">{isEdit ? "Edit Order" : "New Order"}</h1>
        </div>
        ...
        <Card>
          ...
          <CardContent className="grid gap-4">
            <div className="grid gap-2">
              ...
              <Input id="buyerName" value={buyerName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setBuyerName(e.target.value)}
                placeholder="e.g. NFA Regional Office" />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="buyerCompany">Company</Label>
              <Input id="buyerCompany" value={buyerCompany} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setBuyerCompany(e.target.value)}
                placeholder="e.g. National Food Authority" />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="buyerContact">Contact Number *</Label>
              <Input id="buyerContact" value={buyerContact} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setBuyerContact(e.target.value)}
          ...
          <CardContent className="grid gap-4">
            <div className="grid gap-2">
              ...
              <Select value={cropTypeID} onValueChange={setCropTypeID}>
                ...
                <SelectContent>
                  {cropTypes.map((c) => (
                    <SelectItem key={c.cropTypeID} value={String(c.cropTypeID)}>{c.cropName} ({c.category})</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Input id="qty" type="number" min="1" value={requestedQuantity}
            ...
            <div className="grid gap-2">
              ...
              <Input id="orderDate" type="date" value={orderDate}
        ...
        <div className="flex justify-end gap-3 mt-6">
          <Button variant="outline" onClick={() => navigate("/admin/orders")} disabled={loading}>Cancel</Button>
          <Button onClick={handleSubmit} disabled={loading || !canSubmit} id="submit-order-btn">
            {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />{isEdit ? "Saving…" : "Creating…"}</>
